...
socket.on('request', data => {
	...
	if (reqChannel == null) {  // does not exist in channel list, so it's a new p2p channel!
		// ask me to accept or reject user request
		if (confirm(`Bạn có muốn cho phép <${reqUser.username}> nhắn tin với bạn không?`) == false) {
		...
		// ask me to accept or reject user request
		if (confirm(`Bạn có muốn cho phép <${reqUser.username}> tham gia vào kênh <${reqChannel.name}> không?`) == false) {
			socket.emit("reject", { to: data.from, channel: data.channel });
			return;
		}
	}
...
socket.on('reject', data => {
	...
	if (data.p2p)
		alert(`Yêu cầu nhắn tin với <${admin.username}> đã bị từ chối. ${reason}`);
	else
		alert(`Yêu cầu tham gia nhóm <${data.channel}> đã bị từ chối. ${reason}`);
...
function newMessage() {
	...
	if (currentChannelName == null || currentChannelName == '') {
		alert("Chọn một người dùng/nhóm để gửi tin nhắn!");
		return false;
	}
...
(function ($) {
	...
	[ Add channel button ]*/
	$("#addchannel").on("click", () => {
		var name = prompt("Nhập tên kênh chat:", "Channel");
		if (name) {
			...
			if (createChannel(name, false)) {
				...
				alert(`Kênh chat <${name}> đã tồn tại`);
			}
		}
	})
